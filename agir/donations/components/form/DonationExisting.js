...
import {TYPE_CNS, TYPE_DEPARTMENT, TYPE_GROUP} from "@agir/donations/common/allocations.config";
...
export default function DonationExisting({subscription}) {
    ...
    const currentGroup = subscription.allocations?.find((alloc) => alloc.type === TYPE_GROUP)?.group;

    function setupAlloc(type) {
        return subscription.allocations?.find((alloc) => alloc.type === type)?.amount ?? 0
    }

    function setupCurrentSubscription() {
        const remainsAmount = subscription.amount - (setupAlloc(TYPE_CNS) + setupAlloc(TYPE_DEPARTMENT) + setupAlloc(TYPE_GROUP))
        update({
